...
def validate_orders(orders: list[dict]) -> list[dict]:
    ...
    for i, order in enumerate(orders):
        ...
        if not order.get("address") or order["address"] == "ไม่ระบุที่อยู่":
            logger.warning(
                f"Order #{i + 1} ({order.get('customer', 'unknown')}): ไม่มีที่อยู่ — ข้าม"
            )
            continue
        ...
        if order.get("weight", 0) <= 0:
            logger.warning(
                f"Order #{i + 1} ({order['customer']}): น้ำหนัก <= 0 — ใช้ค่าเริ่มต้น 1.0"
            )
            order["weight"] = 1.0
        ...
        if (
            ...
            logger.warning(
                f"Order #{i + 1} ({order['customer']}): พิกัดไม่ถูกต้อง lat={lat}, lng={lng}"
            )
            order["lat"] = None
            order["lng"] = None
