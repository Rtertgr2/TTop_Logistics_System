...
def generate_all_routes_manifest_excel(routes: list[dict]) -> bytes:
    ...
    # Header Section
    ws_summary.cell(row=1, column=1, value="🚛 รายงานสรุปแผนการจัดขนส่งสินค้าประจำวัน").font = header_font
    ws_summary.cell(row=2, column=1, value=f"จำนวนรถที่ใช้งาน: {len(routes)} คัน | รวมจุดส่งทั้งหมด: {sum(len(r.get('stops', [])) for r in routes)} จุด").font = sub_font
    
    # Summary Table Headers
    sum_headers = ["ลำดับ", "คันที่", "ทะเบียนรถ", "พนักงานขับรถ", "ความจุสูงสุด (kg)", "น้ำหนักบรรทุกจริง (kg)", "% ความจุ", "จำนวนจุดส่ง"]
    ...
    for idx, r in enumerate(routes, 1):
        ...
        # Title
        ws.cell(row=1, column=1, value=f"📋 ใบสั่งงานจัดส่งสินค้า — {v_name} (ทะเบียน {r.get('plate', '-')})").font = Font(name="Cordia New", size=18, bold=True, color="1E3A8A")
        ws.cell(row=2, column=1, value=f"พนักงานขับรถ: {r.get('driver', '-')} | ความจุรถ: {r.get('capacity', 0)} kg | น้ำหนักจัดส่งรวม: {r.get('total_weight', 0)} kg").font = sub_font

        # Manifest Headers
        man_headers = ["ลำดับจุดส่ง", "เลขที่ SO", "ชื่อลูกค้า / ร้านค้า", "สถานที่จัดส่งสินค้า", "เขต / โซน", "น้ำหนัก (kg)", "ลายเซ็นผู้รับสินค้า"]
        ws.append([]) # Row 3
