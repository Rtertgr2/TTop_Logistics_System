...
def get_distance_matrix_real(orders: list[dict], depot: dict = None) -> list[list[float]]:
    ...
    if SNAP_TO_ROAD_ENABLED:
        snapped_points = snap_nearest_roads(all_points, enabled=True)
        all_points = [
            {**point, "lat": snapped["lat"], "lng": snapped["lng"]}
            for point, snapped in zip(all_points, snapped_points)
        ]

    # Cache key generated from points list
    cache_key = (
        SNAP_TO_ROAD_ENABLED,
        tuple((p.get("lat"), p.get("lng"), p.get("address")) for p in all_points),
    )
