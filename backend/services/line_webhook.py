...
def verify_line_signature(body: bytes, signature: str) -> bool:
    ...
    if not LINE_CHANNEL_SECRET:
        logger.warning("LINE_CHANNEL_SECRET ไม่ได้ตั้งค่า — ข้ามการ verify signature")
        return True
...
async def line_webhook(request: Request):
    ...
    if LINE_CHANNEL_SECRET:
        if not signature:
            logger.warning("Missing X-Line-Signature header but LINE_CHANNEL_SECRET is set")
            raise HTTPException(status_code=403, detail="Missing signature")
        if not verify_line_signature(body, signature):
            logger.warning("LINE signature mismatch — ตรวจสอบ LINE_CHANNEL_SECRET ให้ตรงกับ console")
            raise HTTPException(status_code=403, detail="Invalid signature")
    else:
        logger.debug("LINE_CHANNEL_SECRET not configured — skipping signature verification (dev mode)")
